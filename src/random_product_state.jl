...
"""
function random_product_state(sites; nup=nothing, ndn=nothing)
    N = length(sites)
    if N == 0
        error("empty sites")
    end
    ...
    if hastags(sites[1], "S=1/2") || hastags(sites[1], "Spinhalf")
        ...
        if nup === nothing
            return [rand(Bool) ? up : dn for _ in 1:N]
        else
            if nup > N
                error("nup=$(nup) exceeds N=$(N) for S=1/2")
            end
            states = fill(dn, N)
            states[randperm(N)[1:nup]] .= up
            return states
        end
    ...
    elseif hastags(sites[1], "tJ")
        ...
        dn  = local_state_index(sites[1], "Dn")
        if nup === nothing
            return [rand([emp, up, dn]) for _ in 1:N]
        else
            if nup + ndn > N
    ...
    elseif hastags(sites[1], "Electron")
        ...
        if nup === nothing
            return [rand([emp, up, dn, updn]) for _ in 1:N]
        else
            if nup > N || ndn > N
                error("nup and ndn must be ≤ N=$(N) for Electron")
            end
            up_sites = Set(randperm(N)[1:nup])
            dn_sites = Set(randperm(N)[1:ndn])
