...
does not exist. Warns if the file is corrupt.
"""
    ...
    try
        h5open(filename, "r") do f
            if !haskey(f, "product_state")
                @warn "Checkpoint file exists but contains no `product_state` dataset, starting fresh." filename
                return 0
            end
            length(read(f["product_state"]))
        end
    catch e
        @warn "Failed to read checkpoint file, starting fresh." filename exception=e
        return 0
    end
end
